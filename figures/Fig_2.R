...
################################################################################
#load data
kelp_swath_processed <- read.csv(here::here("output", "mpla_data", "processed", "kelp_swath_counts_CC.csv"))%>%
  janitor::clean_names()
...
################################################################################
#Fig. 3: Mean density of kelp species in Central Calif ornia fitted with a Generalized Additive Model over time (2007-2022). A: Macrocystis pyrifera as # of individuals/m-2, B: Macrocystis pyrifera as # of stipes/m-2
...
a4pt2 <- ggplot(data = kelp_swath_region_ind_site %>% filter(region == "Carmel"), 
                aes(x = year, y = mean_density, color = kelp_type, fill = kelp_type)) +
  mhw_rect +
  ssw_line+
  geom_point(size = 1.5, alpha = 0.4) +
  geom_smooth(method = "gam",
              formula = y ~ s(x, bs = "cr", sp = 0.05),
              se = TRUE,
              linewidth = 0.8) + 
  scale_color_manual(values = kelp_colors, labels = kelp_labels) +
  scale_fill_manual(values = kelp_colors, labels = kelp_labels) +
  scale_x_continuous(breaks = seq(2001, 2024, by = 1)) +
  coord_cartesian(ylim = c(0, 1)) +                        # zooms without dropping data
  labs(x = "Year",
       y = expression("Mean density (individuals m"^-2*")"),
       title = "A. Carmel") +
  shared_theme + 
...
ggplot(data = kelp_swath_region_ind_site %>% filter(region == "Carmel"),
       aes(x = year, y = mean_density, color = kelp_type, fill = kelp_type)) +
  mhw_rect +
  ssw_line+
  geom_line(linewidth = 0.8) +
